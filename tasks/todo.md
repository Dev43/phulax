...
7. **Track E ABI swap.** Replace `agent/src/abis/FakeLendingPool.ts` import to read from `contracts/generated/wagmi.ts`. **Larger than billed.** A literal swap fails `agent typecheck` because the deployed `FakeLendingPool.sol` (chain id 16602) does **not** expose `sharePrice`/`utilizationBps`/`totalSupply`/`totalReserves`/`totalBorrows` — those getters never existed on-chain; the agent's narrow ABI was modelled on the spec in §6 Tier 1 before the contract evolved into the five-vuln Aave-shape. The real surface is per-asset/per-user `supplied`/`borrowed`/`balanceOf(asset,user)` (see `contracts/src/pools/FakeLendingPool.sol`). Two ways forward, pick one before the dry run:
   - **(a)** Add the five aggregate getters to `FakeLendingPool.sol`, redeploy, rotate addresses in `web/.env.local` + `agent/.env` + KH workflow JSON. Lowest-touch on agent side; high-touch on deployment.
   - **(b)** Rewrite `agent/src/detection/{hydrate,invariants}.ts` to derive aggregates from the real surface — sum `balanceOf(asset, *)` across known assets/users for `totalSupply`, etc. — and define `sharePrice` as `totalSupply / totalBorrows` (or drop monotonic-share invariant entirely, since this pool is 1:1). Lowest-touch on contracts; medium-touch on agent.
   Either way the swap has to come with the matching code change. Tracked here so we don't re-discover this at minute 23 of the demo. *Confirmed 2026-05-02.*
