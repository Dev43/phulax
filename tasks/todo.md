...
- **Agent** (`agent/`): stateless HTTP detection service with two endpoints (`POST /detect-features` for tier 1/2/3, `POST /decide` for aggregation). Pure detection pipeline (4 tiers) + risk aggregator. **No signing surface, no chain subscription, no SSE** — KeeperHub owns the per-block loop and signs the withdraw. 8/8 vitest fixtures green; `tsc --noEmit` clean.
...
- **KeeperHub bridge** (submodule `keeperhub/`, branch `feature/0g-integration`): chain seeds (16602 + 16661), `0g-storage` plugin (kv-get / kv-put / log-append over Flow contract), `0g-compute` plugin (sealed-inference action). **Workflow JSON: `workflows/phulax-guardian.workflow.json` in the parent repo** (kept out of the submodule per `de4ebab8` upstream-PR cleanup). 10-node Block-trigger shape: `Block` (every Galileo block) → `web3/query-transactions` (filter pool `withdraw` selector) → `Condition` (matchCount > 0) → `HTTP /detect-features` (agent service, tier 1/2/3) → `Condition` (hasCandidate) → `HTTP /classify` (inference, tier 4 — this is the slot where 0G sealed inference plugs in once it serves our LoRA) → `HTTP /decide` (agent service, aggregate) → `Condition` (fire) → `web3/write-contract` (`PhulaxAccount.withdraw`, signed by KH org wallet) → `0g-storage/log-append`. KeeperHub fully owns the loop; the agent service holds zero private keys.
...
1. **Wire WSS into KeeperHub.** ✅ **Shipped 2026-05-02.** `CHAIN_RPC_CONFIG` JSON appended to `keeperhub/.env`; jsonKey is `0g-galileo` (matching `CHAIN_CONFIG[16602]` in `lib/rpc/rpc-config.ts`, *not* `0g-testnet` — that string never existed in the registry). Carries `primaryRpcUrl` (https), `fallbackRpcUrl`, `primaryWssUrl: "wss://evmrpc-testnet.0g.ai/ws/"` (trailing slash), `isTestnet: true`. Validated by `bash -c 'set -a; . ./.env; ...'` + `JSON.parse` round-trip. No code changes.
2. **Run the block-time + RPC-latency measurement.** ✅ **Done 2026-05-02** (`pnpm tsx scripts/0g/measure-block-time.ts --blocks 100 --wss-secs 300` with `CHAIN_ZERO_G_GALILEO_PRIMARY_WSS=wss://evmrpc-testnet.0g.ai/ws/` set inline — the script reads the individual env var, not the `CHAIN_RPC_CONFIG` JSON from A1, so the JSON config doesn't propagate here). Numbers:
   - **Block time** (100-block window): avg 0.43s, p50 0s, p95 1s — sub-second blocks.
   - **`eth_getBlockByNumber` HTTPS latency**: p50 148ms, p95 209ms (n=101).
   - **WSS `eth_subscribe("newHeads")`** over 5 min: 690 heads received, **0 drops**, head gaps p50 498ms, p95 537ms.
   - **Detection budget verdict**: with ~430ms blocks vs ~210ms p95 RPC, the `Block` trigger + `query-transactions` round-trip lands well within next-block. The WSS gap p95 (~540ms) is one block-time ahead of the p95 RPC, so SSE-driven UI updates won't lag KH workflow firings. Demo narrative is still "next-block exit," not "same-block."
3. **Host `inference/server.py` publicly.** ✅ **Stub mode shipped 2026-05-02** at `https://phulax-inference.fly.dev` (Fly.io, iad, `auto_stop_machines = "stop"`). HMAC secret already set via `fly secrets`; URL wired into `agent/.env.example`. **Weights bake-in file-prep landed 2026-05-02; deploy in flight.**
   - `inference/Dockerfile` is now multi-stage (`base` → `stub` | `transformers`). Default target is `stub` (tests stay fast, image stays small). The `transformers` target `COPY ml/artifacts/merged /app/model` + `ENV PHULAX_MODEL_DIR=/app/model`, so `/healthz` reports `phase=transformers` + a real `model_hash` on boot.
   - `.dockerignore` keeps the broad `ml/artifacts` deny but allows `ml/artifacts/merged` + `ml/artifacts/merged/**` through so the merged checkpoint ships in the build context. `lora/`, `og-ft/`, `v1_lora/`, `v2_lora/` stay excluded.
   - `inference/fly.toml` bumped to `[vm].memory = "2gb"` and `[build].target = "transformers"` so `fly deploy` picks the weighted stage by default.
   - Confirmed `ml/artifacts/merged/` is present locally (~954 MB: `model.safetensors` 988 MB raw + tokenizer + config + chat template).
   - Deploy command (run from repo root): `fly deploy --config inference/fly.toml --dockerfile inference/Dockerfile --remote-only`. Note the Depot remote-builder context push is bandwidth-bound (~350 KB/s observed for the ~1 GB merged dir → expect ~30–45 min, not the 5–10 min originally estimated). Image lands ~1.7 GB compressed.
   - **Still open** (after the deploy completes): `curl https://phulax-inference.fly.dev/healthz` should report `phase: "transformers"` + a 64-char `model_hash` (not `"stub"`); a real `/classify` call should return `tag` in `{SAFE, RISK}` with non-zero `p_nefarious` on a known-bad fixture; that `model_hash` then needs to match what `python -m upload.og_storage` records for the published merged-weights CID — same hash on both sides is what makes publish-and-replay verifiable. (A5 still has to run for the comparison to be possible.) *Owner: Track D.*
4. **Confirm KH org wallet == on-chain `agent` role.** No more local `AGENT_PRIVATE_KEY` — KH's `web3/write-contract` step signs with the active organization wallet (Turnkey-custodied, looked up via `getOrganizationWalletAddress` in `keeperhub/lib/para/wallet-helpers.ts`). The on-chain `agent` on `PhulaxAccount` is `0x47d3CF2a314aeF4Da43dB8eBC7Eb818bF2496260` — whichever KH org you run the workflow under MUST have that as its active wallet. Verify before A8: KH UI → Organization → Wallets, or query `mcp__keeperhub-*__getOrganizationWallet`. If it doesn't match, either switch orgs or call `cast send <PhulaxAccount> "setAgent(address)" <kh-org-wallet>` from the owner key (with `--priority-gas-price 2gwei` per the Galileo minimum).
5. **Upload artifacts to 0G Storage and populate `ml/artifacts.json`.** ✅ **Shipped 2026-05-02.** `ml/artifacts.json` written; publish-log entry on-chain at stream `0x7070…7002` (rootHash `0x7c24…9196`, txHash `0xce46…605a`).
   - **Pinned with rootHashes** (5 small merged files + adapter + dataset + eval report + 6 harness files + index manifest = 14 blobs total).
   - **`model.safetensors` (988 MB) skipped** — recorded as `{sha256: 0x0f11…3676, size: 988097536, skipped: "size > OG_MAX_BLOB_BYTES"}`. The 0G SDK's Merkle-root construction at 256 bytes/leaf was CPU-bound for ~30 min on the full safetensors with no progress; we cap at 64 MB per blob (`OG_MAX_BLOB_BYTES`) and let the publish-log anchor via the offline sha256 instead. The Fly.io inference image already serves the merged weights directly, so verifiability falls back to "operator publishes sha256, anyone can replay against the same hash" — slightly weaker than rootHash-pinned but defensible. Bump the env var to upload more, but expect long CPU times.
   - **Two real bugs fixed during the run** so the script doesn't trip the next time:
     - `ml/scripts/package.json` had `@0gfoundation/0g-ts-sdk@^0.5.0` — that version range never existed. Bumped to `1.2.6` (matches keeperhub submodule).
     - `ml/scripts/og.mjs` left the 0G SDK's stdout chatter pointed at fd 1, which polluted the JSON channel that `ml/og_client.py` parses. Now the SDK chatter is rerouted to stderr and only the final JSON result is written to stdout.
     - `ml/upload/og_storage.py` looked for `adapter.safetensors`; PEFT writes `adapter_model.safetensors`. Renamed.
     - Added `dotenv.load_dotenv(ml/.env)` at the top of `og_storage.py` — its own error message promised that path; nothing was actually loading it.
   - Manifest carries `weights_sha256` alongside `weights_cid` so the iNFT metadata anchor (B9) can use either. *Owner: Track C.*
6. **Import `workflows/phulax-guardian.workflow.json` and fill the placeholders.** Replace `PHULAX_SVC_URL` with the public URL of the agent service (`/detect-features` and `/decide` are the two endpoints KH calls), replace `PHULAX_INFERENCE_URL` with the inference host (already `https://phulax-inference.fly.dev` per step 3), and replace `REPLACE_WITH_RECEIPT_STREAM_ID` with a 0G Storage stream ID for the receipt log (provision via `0g-storage/log-append` test run, or reuse `PHULAX_INCIDENT_LOG_STREAM_ID` from `agent/.env.example`). Verify by importing into the KH dev project and clicking through field validation. *Owner: Track A.*
7. **Track E ABI swap.** ✅ **Shipped 2026-05-02 (option b).** `agent/src/abis/FakeLendingPool.ts` now mirrors the real deployed surface (`admin`, `getAssetPrice`, `balanceOf(asset,user)`, `borrowedOf`, plus the five write selectors). Tier-1 invariants rewritten: dropped fictional `sharePrice`/`utilizationBps`/`totalSupply`/`totalReserves`/`totalBorrows` checks (cannot be derived without off-chain user enumeration), replaced with two real per-tx invariants — **admin sweep** (vuln #4: `selector == withdrawReserves` + `from == admin`) and **reentrancy** (vuln #2: pool ERC-20 reserve dropped > visible `withdraw` amount + 5% slack). Hydrate now reads `pool.admin()` + `IERC20(asset).balanceOf(pool)` at blockN/N-1; the `prevSharePrice` parameter is gone from `hydrate()` and the `/detect-batch` request body. New `SignalKind`s `invariant.adminSweep` / `invariant.reentrancy` replace the three retired kinds. Fixtures rewritten (`adminSweep_rug` + `reentrancy_drain` cover tier-1 short-circuit; mango/benign/classifier-only/vector-only kept). `pnpm typecheck` clean, `pnpm test` 8/8 green. *Live runtime invariants now functional against the deployed pool — unblocks A8.*
8. **End-to-end testnet dry run.** Steps 1–7 must be green first.
   - **Agent service**: `cd agent && pnpm dev` (or deploy publicly so KH can reach it). `curl http://localhost:8787/health` returns `{ok: true}`. Smoke-test `/detect-features` with a fixture: `curl -XPOST localhost:8787/detect-features -H content-type:application/json -d '{"account":"0xA70060…","adapter":"0x0c39fF…","txs":[]}'` returns `{hasCandidate: false}`.
   - **Inference**: `curl https://phulax-inference.fly.dev/healthz` returns `{phase: "...", model_hash: "..."}`.
   - **Workflow**: import `workflows/phulax-guardian.workflow.json` into the KH dev project on `feature/0g-integration`. Replace the three placeholders per A6. Save → KH activates the `Block` trigger on chain 16602.
   - **Benign run**: fire a `supply` tx through `web/` or `cast`. The Block trigger fires, `query-transactions` filters out non-`withdraw` txs (matchCount=0), the `Has matching txs?` Condition skips the rest. Confirm via the KH run history.
   - **Demo attack**: script the oracle-manipulation drain from `contracts/test/ExploitReplay.t.sol` against the deployed pool:
     ```bash
     ...
     # then borrow against inflated collateral, then withdraw to drain
     ```
   - Expect: KH workflow fires on the block containing the attacker's `withdraw`, `/detect-features` returns a hasCandidate=true tier-1 short-circuit (reentrancy invariant), `/classify` runs (kept for the receipt — even though tier-1 already short-circuits the score), `/decide` returns `fire: true`, `web3/write-contract` lands `PhulaxAccount.withdraw(adapter)` from `0x47d3CF…`, receipt appended to 0G Storage Log.
   - Confirm: victim recovers ≥ 99 % of principal in `DemoAsset.balanceOf(owner)` within ≤ 1 block of the attacker tx. **This is the demo.**
...
11. **Web Phase 3 — live updates.** The agent service no longer streams (SSE was dropped when the per-block loop moved to KH). Pick one: (a) tail the 0G Storage receipt log via the existing `agent/og/log.ts` and re-emit as SSE from a thin endpoint, or (b) poll KH's run-history API every 2s, or (c) fall back to polling `agent/server.ts:GET /incidents/:account` (already exists, served from 0G Storage). The dashboard's `lib/mock.ts` shape is unchanged either way — only the data source flips. *Owner: Track F.*
...
## Review (2026-05-02 — worktree `dmux-1777742798586`)

- **A5 code complete (run still pending).** `ml/upload/og_storage.py` now (a) pins `ml/artifacts/lora/adapter.safetensors` when present and records its rootHash in the manifest, and (b) appends a `model_publish` entry to a dedicated 0G Storage Log stream (`OG_PUBLISH_STREAM_ID`, default `0x70…02`) on every run. The entry packages `{kind, model_hash, template_version, dataset_sha256, weights_cid, adapter_cid, eval_cid, provider, task_id, timestamp}` per §10.1 Stage C step 4 — that's the publish-and-replay receipt the strategy depends on. `provider`/`task_id` come from `ml/artifacts/og-ft/run.json` (null when local PEFT was used); `weights_cid` prefers the GGUF rootHash and falls back to `model.safetensors`/`pytorch_model.bin`. Manifest now carries `publish_log: {rootHash, txHash, streamId, key, entry}` so consumers can verify the log without scanning the stream. Syntax-checked with `ast.parse`; full run gated on `OG_PRIVATE_KEY` + `ml/artifacts/merged/` (not present in this worktree).
- **A7 swap is *not* a one-file change — flagged inline above.** Tried `export { fakeLendingPoolAbi } from "../../../contracts/generated/wagmi"` after `pnpm --filter @phulax/contracts wagmi`. `pnpm typecheck` failed against the agent's invariants tier: the deployed `FakeLendingPool` never exposed `sharePrice`/`utilizationBps`/`totalSupply`/`totalReserves`/`totalBorrows`. The narrow ABI in `agent/src/abis/FakeLendingPool.ts` was a fiction the contract never satisfied; tests pass only because every fixture builds `InvariantSnapshot` directly (no `readContract`). Reverted the swap to keep tests green and updated §14.A.7 with the two ways forward. **The runtime invariants tier is non-functional against the deployed pool today** — A8's "kv-get reads index → HTTP step calls inference" path works because hydrate.ts only fails on the tier-1 reads, but the score never benefits from invariants. Either redeploy with the aggregate getters (option a) or rewrite hydrate.ts/invariants.ts (option b) before the demo dry run.

- **A7 resolved (2026-05-02, option b).** Rewrote `agent/src/abis/FakeLendingPool.ts` to mirror the real deployed surface (admin, getAssetPrice, balanceOf(asset,user), borrowedOf, plus the five write selectors with their actual arities — `supply` and `withdraw` take 3 args, not 2). `InvariantSnapshot` shape replaced: out go `sharePrice`/`utilizationBps`/`totalSupply`/`totalReserves`/`totalBorrows`/`prevSharePrice`; in come `selector`/`fromIsAdmin`/`asset`/`poolReserve`/`poolReservePrev`/`txAmount`. Two new tier-1 invariants in `agent/src/detection/invariants.ts`:
  - **Admin sweep** — `selector == withdrawReserves(address,address)` (computed via `viem.toFunctionSelector` at module load, not hardcoded) AND `raw.from == pool.admin()`. Maps to vuln #4. delta=0.8.
  - **Reentrancy** — `selector == withdraw(address,uint256,address)` AND pool's `IERC20(asset).balanceOf(pool)` dropped between blockN-1 and blockN by more than `txAmount + 5%`. Maps to vuln #2. delta=0.8.
  `hydrate.ts` no longer takes `prevSharePrice`; instead it reads `pool.admin()` + `IERC20(asset).balanceOf(pool)` at blockN and blockN-1 in parallel with the existing oracle reads. The `/detect-batch` POST body lost `prevSharePrice` accordingly — KH workflow JSON doesn't reference it (just pulls the field through agent SSE). New `SignalKind`s: `invariant.adminSweep`, `invariant.reentrancy` (the three retired kinds were the only ones removed). Fixtures retired: `cream_sharePriceDrop`, `euler_utilizationOverflow`, `generic_accountingDrift`. New fixtures: `adminSweep_rug`, `reentrancy_drain`. `pnpm typecheck` clean, `pnpm test` 8/8 green (6 fixture cases + 2 purity tests). The deferred risk — that the agent's tier-1 was a fiction against the testnet — is closed; A8 dry run can now exercise all four tiers end-to-end. The vuln #1 demo still fires through tier-2 (oracle) + tier-3 (vector) + tier-4 (classifier) per the existing `mango_oracleManipulation` fixture path.

- **Single-box deploy stack landed (2026-05-02).** New `docker-compose.yml` at the repo root brings up Caddy + KeeperHub (db + localstack + app) + web + agent + inference on one host. `deploy/Caddyfile` puts everything behind HTTP basic_auth (`ethglobal` / bcrypt-hashed password) and routes by path:
  - `/` (catch-all) → KeeperHub at root, **no upstream basePath patch**.
  - `/web*` → phulax web; `web/next.config.mjs` now reads `NEXT_PUBLIC_BASE_PATH` so dev stays at `/` and the build sets `basePath=/web` via `deploy/Dockerfile.web --build-arg`.
  - `/agent*` → phulax agent with prefix stripped at Caddy, so Fastify routes stay root-relative; new `agent/src/log-buffer.ts` captures pino logs into a 500-line ring buffer and serves a self-refreshing HTML tail at `GET /` (so `/agent` in a browser shows the agent thinking instead of a 404). Required wiring `pino@^9.5.0` as a direct dep and switching `Fastify({ logger: true })` to `Fastify({ loggerInstance: buildLogger() })`.
  - inference is **internal-only** — no public route; agent + KH workflow steps call `http://inference:8000` over the Docker network. Reuses the same `transformers` stage of `inference/Dockerfile` we ship to Fly, so colocation eliminates the Fly cold-start completely.
  - `deploy/Dockerfile.agent` and `deploy/Dockerfile.web` both build from the repo root so the pnpm workspace + lockfile are in scope. Web's `--build-arg NEXT_PUBLIC_BASE_PATH=/web` bakes the prefix at build time (Next.js requires this — basePath is build-time, not runtime).
  - `deploy/.env.example` + `deploy/README.md` document required env, expected memory footprint (~3.5 GB resident → 8 GB VM recommended), and the rebuild path. `pnpm typecheck` clean, `pnpm test` 8/8 green, `docker compose config` and `caddy validate` both clean.
  - **Latency consequence**: with inference colocated on dedicated CPU (vs. Fly shared-cpu-1x), warm calls drop from ~2 min to ~10–30 s. Demo still doesn't gate `withdraw` on the classifier — tier 1/2/3 fire synchronously, classifier writes a corroborating receipt to the 0G Storage Log a few seconds later.
