...
- **KeeperHub bridge** (submodule `keeperhub/`, branch `feature/0g-integration`): chain seeds (16602 + 16661), `0g-storage` plugin (kv-get / kv-put / log-append over Flow contract), `0g-compute` plugin (sealed-inference action). **Workflow JSON: `workflows/phulax-guardian.workflow.json` in the parent repo** (kept out of the submodule per `de4ebab8` upstream-PR cleanup). 3-node webhook-driven shape — `Webhook` trigger → `web3/write-contract` (`PhulaxAccount.withdraw`) → `0g-storage/log-append` receipt. The earlier `per-tx-detection.workflow.json` skeleton claim was aspirational; that file was never committed, and the architecture moved to webhook-driven (detection stays in `agent/`, KH does signing + receipt only) so A4/A6/A8 below need rewriting to match.
